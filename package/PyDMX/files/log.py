...
def log(src:str,tpe:str,txt:str): #Log function for GUI functions
	...
	try:
		with open(vars.logfilepath,"a",encoding="UTF-8") as fle:
			fle.write(f"<button onclick='highlightline(\"{vars.logid}\")' id='b-{vars.logid}'><b>H</b></button><span style='color:white'> {vars.logid}: </span><span style='color:{mdclr}' id='s-{vars.logid}'><b>[{vars.progname}/{src}]</b> <i><b>{tpe.upper()}</b></i>: {txt}</span><br>\n")
	except: pass
